...
def photo_eat(status, num):
    ...
    for i in range(num):
        ...
        os.system("raspistill -o %s.jpg -w 1280 -h 720" % local)
        print("raspistill -o %s.jpg -w 1280 -h 720" % local)
    time.sleep(1)
...
def photo(status, num):
    ...
    for i in range(num):
        local = home + status + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime()))
        os.system("raspistill -o %s.jpg -w 1280 -h 720" % local)
        print(num+"raspistill -o %s.jpg -w 1280 -h 720" % local)
    time.sleep(1)
